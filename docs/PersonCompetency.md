# PersonCompetency


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**personId** | **number** |  | [default to undefined]
**level** | [**PersonCompetencyAllOfLevel**](PersonCompetencyAllOfLevel.md) |  | [optional] [default to undefined]
**skillId** | **number** |  | [default to undefined]
**createdAt** | **string** | Timestamp of when the object was created. | [default to undefined]
**updatedAt** | **string** | Timestamp of when the object was last updated. | [default to undefined]

## Example

```typescript
import { PersonCompetency } from 'runn-typescript-sdk';

const instance: PersonCompetency = {
    personId,
    level,
    skillId,
    createdAt,
    updatedAt,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
